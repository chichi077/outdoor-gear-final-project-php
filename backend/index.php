    ...
    try{
        ...
        switch($_SERVER["PATH_INFO"]){
            case "/reg":
                ...
                $selectCmd = "SELECT email FROM users_tb WHERE email='".$_POST["email"]."'";
                ...
                $pass = password_hash($_POST["password"], PASSWORD_BCRYPT, ["cost"=>10]);   //hash and salt
                $insertCmd =$dbCon->prepare("INSERT INTO user_tb (fname, lname, email, password, user_type) VALUES (?,?,?,?,?)");
                $insertCmd->bind_param("ssssi", $_POST["fname"], $_POST["lname"], $_POST["email"], $pass, $user_type);
                $insertCmd->execute();
                $db_obj->db_close();
