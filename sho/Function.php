<?php
    ...
    function Session_Hanlder($sid){ //to manage session time
        session_id($sid);
        session_start();
        if(isset($_SESSION["time_out"]) && $_SESSION["time_out"] > time()){ //if session is set and time is over, extend the time
